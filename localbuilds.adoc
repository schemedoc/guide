...
Since the MacOS system `gcc` is Apple's `clang` wrapper, Homebrew
installs GCC as `gcc-9` (where `9` is GCC's major version number). You
can find the command name with:
